...
## Phase 42: Default read tools with access-controlled sensitive stages

Goal: adjust the Phase 39 access-control semantics so read-only context tools are ergonomic by default. `Read`, `Grep`, and `Glob` are baseline AI context-gathering tools and should not have to be repeated in every stage's `tools` list. Write-capable tools and non-read tools remain stage-gated. Sensitive stages can still restrict read access by defining `access.read`.

This phase supersedes the strict Phase 39 interpretation that `Read` / `Grep` / `Glob` must always be present in `stage.tools` before they can run. Do not remove the path-policy enforcement from Phase 39; only change the tool-name default for read-only tools.

Hard acceptance rules for this phase:

1. `Read`, `Grep`, and `Glob` are default read-only tools. When a run is active, they may pass the tool-name allowlist even if omitted from `stage.tools`.
2. If the current stage has no `access.read` policy, `Read`, `Grep`, and `Glob` should be allowed for normal context gathering. Existing write/tool gating still applies to all non-read tools.
3. If the current stage has `access.read`, `Read`, `Grep`, and `Glob` must still be checked against the read path policy. Default read permission must not bypass `allow` / `deny`, unresolved variables, project-root escape checks, missing path/search-root fail-closed behavior, or directory search-root deny-intersection checks.
4. `Write`, `Edit`, `MultiEdit`, and `NotebookEdit` must remain explicitly stage-gated by `stage.tools` and then checked against `access.write` when present. Do not make write tools default.
5. `Bash`, `PowerShell`, `WebSearch`, task tools, and all other non-read tools keep their existing `stage.tools` semantics. Do not expand this phase into a general "allow all safe tools" feature.
6. Empty `tools: []` keeps its existing meaning of unrestricted tools, but `access.read` / `access.write` must still restrict covered file tools when present.
7. Documentation and examples must state the practical rule clearly: "read defaults open; write remains explicit; sensitive stages add `access.read`."
8. Tests must prove both sides of the behavior: read tools omitted from `tools` are allowed without `access.read`, and the same omitted read tools are blocked by `access.read` when they target denied or unallowed paths. Tests must also prove write tools omitted from `tools` remain blocked.
9. Do not claim this is an OS sandbox. A permitted script/shell command can still read/write internally according to the Phase 39 boundary.
10. Do not satisfy this phase with pytest-only verification. Unit and hook tests are required, but the final verification must also use the real CLI from an external project directory outside `D:\Tool\auto_workflow`.
11. The external smoke demo must be created under `D:\2026_zju\test` in a fresh subdirectory, for example `D:\2026_zju\test\stageflow_phase42_demo_<timestamp>`. It must not reuse `D:\Tool\auto_workflow`, must not depend on pytest fixtures, and must not accidentally discover the StageFlow source checkout as the project root.
12. The external smoke demo must use the installed `stageflow` command from PATH if available. If PATH resolution is broken, the agent may use `python -m stageflow` only as a fallback and must record that fallback explicitly. The demo still must exercise the CLI/hook behavior as an end user would, not by importing Python functions directly.
13. The external smoke demo is considered "runs through" only if all of these are true: `stageflow init` creates a new `.stageflow` project in the demo folder; a tiny custom YAML can be written there; `stageflow start <stage>` starts that YAML's first test stage; simulated `stageflow hook` calls show omitted `Read` / `Grep` are allowed for ordinary reads, `access.read.deny` blocks `.env` or `secrets/**`, and an omitted write tool such as `Write` is still blocked. The agent must print or record the exact commands and observed allow/block results.
14. The external smoke demo should stay very small. Use at most a few stages and a few tiny files such as `README.md`, `.env`, `secrets/key.txt`, `artifacts/out.txt`, one tiny buggy Python file, and one tiny test file. Do not build a large sample workflow or require a browser/editor server for this phase.
15. The tiny work item should use a QuixBugs/MBPP-style task, not a large SWE-bench-style issue. Recommended demo: create `buggy_math.py` with a one-line bug in a simple function such as `is_even(n)` or `clamp(value, low, high)`, plus `test_buggy_math.py` with 3-5 assertions. The AI should be able to inspect context, fix one line, and run the tests. This provides a real "can the workflow run through" check without depending on downloading a full benchmark.
16. The agent executing this phase must not invent the bug itself. The demo bug must come from the preselected bug seed bank below, or from another explicitly cited benchmark source added to this fix plan before execution. Do not create a fresh toy bug during task execution and then "solve" it; that makes the run-through self-authored and invalid.
17. The demo must include only the buggy source, a short task statement, and tests. Do not copy a correct solution file, patch, answer key, or `correct_python_programs` equivalent into the demo folder. If tests are adapted from a benchmark, keep them small and self-contained instead of importing the benchmark's full harness.

Preselected bug seed bank for task-144. A local copy has been prepared at `D:\2026_zju\test\stageflow_bug_bank`. Use exactly one local task folder from that bank unless additional difficulty is explicitly requested; do not make the executing AI search the web for these bugs.

- `QX_GCD`: based on QuixBugs `python_programs/gcd.py`. Source: `https://github.com/jkoppel/QuixBugs/blob/master/python_programs/gcd.py`. Minimal local task: implement/fix `gcd(a, b)` for nonnegative integers; tests should include examples such as `(35, 21) -> 7`, equal inputs, one input zero, and coprime inputs. Do not include the correct implementation text in the task statement.
- `QX_VALID_PARENS`: based on QuixBugs `python_programs/is_valid_parenthesization.py`. Source: `https://github.com/jkoppel/QuixBugs/blob/master/python_programs/is_valid_parenthesization.py`. Minimal local task: fix validation of balanced parentheses; tests should include a balanced nested string, a string that goes negative early, and a string with leftover unmatched opening parentheses.
- `QX_FIND_FIRST`: based on QuixBugs `python_programs/find_first_in_sorted.py`. Source: `https://github.com/jkoppel/QuixBugs/blob/master/python_programs/find_first_in_sorted.py`. Minimal local task: return the first index of `x` in a sorted list or `-1`; tests should include duplicates, missing values, singleton/empty lists, and target at index 0.
- `QX_TO_BASE`: based on QuixBugs `python_programs/to_base.py`. Source: `https://github.com/jkoppel/QuixBugs/blob/master/python_programs/to_base.py`. Minimal local task: convert a positive base-10 integer to base `b`; tests should include base 2, base 10, base 16 with multi-digit output, and a value whose digits would reveal reversed order.

- [x] **task-139**: Update the hook and StageGuard semantics for default read tools. `stageflow hook` and programmatic `StageGuard.check()` must treat `Read`, `Grep`, and `Glob` as default read-only tools at the tool-name layer, then apply `access.read` when present. Preserve strict `stage.tools` checks for write and non-read tools.
- [x] **task-140**: Update the read-access tests. Add hook-level and StageGuard tests where a stage omits `Read` / `Grep` / `Glob` and they are allowed without `access.read`; add paired tests where the same omitted read tools are blocked by `access.read.deny`, `access.read.allow`, missing search root, and directory search-root intersection. Keep existing tests proving `Grep` / `Glob` cannot bypass sensitive path policies.
- [x] **task-141**: Preserve write-tool strictness. Add or update tests proving `Write`, `Edit`, `MultiEdit`, and `NotebookEdit` are blocked when omitted from `stage.tools`, even if their path would pass `access.write`; when present in `stage.tools`, they must still obey `access.write`.
- [x] **task-142**: Update docs and examples. Revise CLI/API docs, default workflow comments, and Ralph/agent guidance to explain the new ergonomic default: read-only context tools are baseline, write tools are explicit, and sensitive stages use `access.read` to limit reads. Include a small YAML example with `tools: [Write]` plus `access.read.deny` showing that read tools need not be listed but still cannot read `.env` or `secrets/**`.
- [ ] **task-143**: Run a real external CLI smoke demo under `D:\2026_zju\test`. Create a fresh folder named like `stageflow_phase42_demo_<timestamp>`, run `stageflow init`, write a tiny `.stageflow/config/stages.yaml` that omits `Read` / `Grep` from `tools` but includes one sensitive stage with `access.read.deny`, then run `stageflow start` and simulated `stageflow hook` stdin calls. Verify and record: ordinary `Read` or `Grep` is allowed when omitted from `tools`; `.env` or `secrets/**` is blocked by `access.read`; `Grep` without a safe search root is blocked in the sensitive stage; `Write` is blocked when omitted from `tools`; `Write` is allowed only in a stage where it is explicitly listed and its path passes `access.write`. This task must not be replaced by pytest.
- [ ] **task-144**: Add the tiny QuixBugs/MBPP-style run-through to the same external demo. In the demo folder, select exactly one item from the local preselected bug seed bank at `D:\2026_zju\test\stageflow_bug_bank` and copy only that task's `buggy_task.py`, `test_buggy_task.py`, `TASK.md`, and `source.txt`. Use StageFlow stages like `inspect -> fix -> verify -> done`. Confirm the agent can read ordinary context by default, write only in the explicit fix stage, and run the tests to completion. The expected proof is not benchmark scoring; it is a small transcript or log showing the selected seed id, `stageflow start`, hook allow/block checks, the failed test before the fix, the one-line or very small fix, the passing test command, and final `stageflow complete` or terminal-stage success. This task is invalid if the executing AI invents its own bug, searches the web for the answer, includes the correct solution in the prompt/files, or skips the pre-fix failing test.
