...
- [ ] **task-141**: Preserve write-tool strictness. Add or update tests proving `Write`, `Edit`, `MultiEdit`, and `NotebookEdit` are blocked when omitted from `stage.tools`, even if their path would pass `access.write`; when present in `stage.tools`, they must still obey `access.write`.
