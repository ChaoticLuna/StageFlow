...
- [x] **task-144**: Add the tiny QuixBugs/MBPP-style run-through to the same external demo. Selected QX_GCD from seed bank. Copied buggy_task.py, test_buggy_task.py, TASK.md, source.txt. Used inspect→fix→verify workflow (3 transitions). Pre-fix: 4/4 FAIL (RecursionError). Fix: one line `gcd(a % b, b)` → `gcd(b, a % b)`. Post-fix: 4/4 PASS. Hook checks verified: Read defaults open in inspect, Write blocked in inspect, Write allowed in fix, Bash(pytest) allowed in verify. Transcript at demo/QUIXBUGS_TRANSCRIPT.md.
