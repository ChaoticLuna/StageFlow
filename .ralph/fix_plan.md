...
- [x] **task-143**: Run a real external CLI smoke demo under `D:\2026_zju\test`. Created `stageflow_phase42_demo`, ran `stageflow init`, wrote custom YAML with `inspect` (tools:[WebSearch], access.read.deny:[.env, secrets/**]) and `build` (tools:[WebSearch,Write], access.write.allow:[artifacts/**]) stages. 11 hook tests verified: default reads allowed, .env/secrets blocked, Write blocked when not in tools, Write allowed with access.write.allow, nested cwd discovery works. Transcript at demo/DEMO_TRANSCRIPT.md.
